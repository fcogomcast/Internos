...
import stpa.utils.ControlHorario.preferencias.PreferenciasException;
...
public class SoapServerHandler implements SOAPHandler<SOAPMessageContext> {
	...
	public static SOAPFaultException createSOAPFaultException(String pFaultString)  throws SOAPException{
		 ...
		 try {
			 ...
			 fault.setFaultCode(new QName(SOAPConstants.URI_NS_SOAP_ENVELOPE, "ControlHorario"));
			 sex= new SOAPFaultException(fault);
		 }catch (Exception ex){
			 throw new SOAPException("Control horario:===>Error grave al construir la excepción de SOAP."+ex.getMessage());
		 }
		 return sex;
	} 
	...
	private synchronized void log(SOAPMessageContext context) throws SOAPException{
		...
		try{
			...
			throw createSOAPFaultException("Error mientras se procesaba la petición del servicio de envío de control horario.");
		}
